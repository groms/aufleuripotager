...
if($manufacturer_id==10) {/*$freezeOrders="true";*/ echo "FORCE_VALIDATION";}
}
...
//jmo patch !!!
if($manufacturer_id==10)
{
	if($order_date_to=="2016-03-27")
	$can_validate=1;
//	else
//	$can_validate=0;
//echo "JMO !!!".$order_date_to."can_validate: ".$can_validate.$_GET['order_date_to'];
}
//
...
//        if (((!$print)&&($preprod))||((!$print)&&($is_week_mode)&&(($forceValidation)||(!$forceValidation)&&(!$has_frozen_orders)))) {
          $recurrences_table = '
	        ...
	        $recurrences_table .= tep_draw_form('valid_recurrences',FILENAME_STATS_MANUFACTURERS, querystring_small(), 'post', 'onsubmit="javascript:document.getElementById(\'submit_btn\').disabled=true;"').
	          '<table border="0" width="100%" cellspacing="1" cellpadding="2" style="BORDER: 2px solid;">';
//echo "JMO: ".$forceValidation.$has_recurrences_to_valid;print_r($HTTP_POST_VARS['checkbox']);
...
//$jmo.=getCustomersByProductBlock();
      $sqljmo = "SELECT prodd.products_name, prodd.`products_id` FROM `products` prod
left join `products_description` prodd on (prod.`products_id`=prodd.`products_id`)
WHERE prod.`manufacturers_id`=10";
      $queryjmo = tep_db_query($sqljmo, 'db_link');
//print_r($queryjmo);
...
//$jmo.="<br/>".tep_format_qty_for_html($curTotalQuantity)." £ ".$curTitleName." £ ".$curLastOrderDate;
//print_r($queryjmo);echo "coucou";
      while ($rowjmo = tep_db_fetch_array($queryjmo))
	{
//print_r($rowjmo);echo "couocu";
		if($rowjmo['products_name']==$curTitleName) $curid=$rowjmo['products_id'];
	}

//print_r($queryjmo);
//foreach($queryjmo as $rowjmo)
//if($rowjmo['products_name']==$curTitleName) $curid=$rowjmo['products_id'];
array_push($jmoa,$curTotalQuantity,$curid);
...
if($manufacturer_id==10)
{
//echo "debut jmo".$jmo."fin jmo";
//echo "<br/>reload: ".$reload;
//echo "<br/>can_validate: ".$can_validate;
//echo "<br/>force_validation: ".$HTTP_GET_VARS['force_validation'];
//echo "<br/>date: ";print_r($HTTP_GET_VARS["order_date_to"]);
//echo "<br/>".$TotalQuantity." ".$TotalPrice."<br/>";
$sjmo=serialize($jmoa);
          $sjmo = str_replace("\"", "'", $sjmo);
/*print_r($jmoa);
echo "<br/>";*/
//print_r($sjmo);
//echo "<br/>open: ".ini_get('allow_url_fopen');
//echo "<br/>include: ".ini_get('allow_url_include');
/****************************************/
...
//
//echo "<br/>can_validate: ".$can_validate;
//echo "<br/>order_date_to: ".$order_date_to;
//echo "select_mode: ".$customer_id;
//if( isset($HTTP_GET_VARS["all_cust_by_prod"])) echo "coucou";
//echo "recurrences: ".print_r($recurrences);
if($customer_id=="all_cust_by_prod" && $HTTP_GET_VARS["facturation_months"]=="")
{
	if(($is_week_mode)&&($has_frozen_orders))
	{
echo 'Valider comme avant, puis cliquer sur Et Hop ! pour enregistrer la commande chez trebara ';
echo '&nbsp';
echo '<a style="padding: 3; margin_left: 100px; margin_right: 100px; align: center; text-align: justify; border: 1px solid black; background-color: aliceblue; color: black; height: 15px; 	width: 300px; font-weight: bold; font-size: 16pt; " href="https://trebara.com/boutique/prestashop/modules/simpleorder/lelocal.php?deliv='.$order_date_to.'&data='.$sjmo.'"><b>Et Hop ! </b></a>';
if(isset($HTTP_GET_VARS["cron"]) && $HTTP_GET_VARS["cron"]==1 /*&& !isset($HTTP_GET_VARS["force_validation"])*/)
       echo '<META HTTP-EQUIV="Refresh" Content="0; URL=https://trebara.com/boutique/prestashop/modules/simpleorder/lelocal.php?deliv='.$order_date_to.'&data='.$sjmo.'">';
	}
	else if(isset($HTTP_GET_VARS["cron"]) && $HTTP_GET_VARS["cron"]==1)
	{
//	       if(!isset($HTTP_GET_VARS["force_validation"]))
//		echo '<META HTTP-EQUIV="Refresh" Content="0; URL=https://trebara.com/boutique/prestashop/modules/simpleorder/cronlocal.php?cron=1&can_validate=1">';
//		else
//		echo '<META HTTP-EQUIV="Refresh" Content="0; URL=https://trebara.com/boutique/prestashop/modules/simpleorder/cronlocal.php?cron=1">';
	}
}
//
}  
