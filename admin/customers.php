...
<? } ?>
  ...
  if (customers_firstname == "" || customers_firstname.length < <? echo ENTRY_FIRST_NAME_MIN_LENGTH; ?>) {
    error_message = error_message + "<? echo JS_FIRST_NAME; ?>";
    error = 1;
  }

  if (customers_lastname == "" || customers_lastname.length < <? echo ENTRY_LAST_NAME_MIN_LENGTH; ?>) {
    error_message = error_message + "<? echo JS_LAST_NAME; ?>";
    error = 1;
  }

<? if (ACCOUNT_DOB == 'true') { ?>
  if (customers_dob == "" || customers_dob.length < <? echo ENTRY_DOB_MIN_LENGTH; ?>) {
    error_message = error_message + "<? echo JS_DOB; ?>";
    error = 1;
  }
<? } ?>

  if (customers_email_address == "" || customers_email_address.length < <? echo ENTRY_EMAIL_ADDRESS_MIN_LENGTH; ?>) {
    error_message = error_message + "<? echo JS_EMAIL_ADDRESS; ?>";
    error = 1;
  }

  if (entry_street_address == "" || entry_street_address.length < <? echo ENTRY_STREET_ADDRESS_MIN_LENGTH; ?>) {
    error_message = error_message + "<? echo JS_ADDRESS; ?>";
    error = 1;
  }

  if (entry_postcode == "" || entry_postcode.length < <? echo ENTRY_POSTCODE_MIN_LENGTH; ?>) {
    error_message = error_message + "<? echo JS_POST_CODE; ?>";
    error = 1;
  }

  if (entry_city == "" || entry_city.length < <? echo ENTRY_CITY_MIN_LENGTH; ?>) {
...
?>
  if (document.customers.elements['entry_state'].type != "hidden") {
    if (document.customers.entry_state.value == '' || document.customers.entry_state.value.length < <? echo ENTRY_STATE_MIN_LENGTH; ?> ) {
...
?>
  ...
  if (customers_telephone == "" || customers_telephone.length < <? echo ENTRY_TELEPHONE_MIN_LENGTH; ?>) {
...
?>
      ...
      <tr><? echo tep_draw_form('customers', FILENAME_CUSTOMERS, tep_get_all_get_params(array('action')) . 'action=update', 'post', 'onSubmit="return check_form();"') . tep_draw_hidden_field('default_address_id', $cInfo->customers_default_address_id); ?>
        <td class="formAreaTitle"><? echo CATEGORY_PERSONAL; ?></td>
      </tr>
      <tr>
...
?>
      ...
      <tr>
        <td class="formAreaTitle"><? echo CATEGORY_COMPANY; ?></td>
      </tr>
      <tr>
        <td class="formArea"><table border="0" cellspacing="2" cellpadding="2">
          <tr>
...
?>
      ...
      <tr>
        <td class="formAreaTitle"><? echo CATEGORY_ADDRESS; ?></td>
      </tr>
      <tr>
        <td class="formArea"><table border="0" cellspacing="2" cellpadding="2">
          <tr>
...
?></td>
      ...
      <tr>
        <td class="formAreaTitle"><? echo CATEGORY_CONTACT; ?></td>
      </tr>
      <tr>
        <td class="formArea"><table border="0" cellspacing="2" cellpadding="2">
          <tr>
...
?></td>
      ...
      <tr>
        <td class="formAreaTitle"><? echo CATEGORY_OPTIONS; ?></td>
      </tr>
      <tr>
        <td class="formArea"><table border="0" cellspacing="2" cellpadding="2">
          <tr>
...
?></td>
      ...
      <tr>
        <td align="right" class="main"><? echo tep_image_submit('button_update.gif', IMAGE_UPDATE) . ' <a href="' . tep_href_link(FILENAME_CUSTOMERS, tep_get_all_get_params(array('action'))) .'">' . tep_image_button('button_cancel.gif', IMAGE_CANCEL) . '</a>'; ?></td>
      </tr></form>
<?
  } else {
?>
      <tr>
        <td><table border="0" width="100%" cellspacing="0" cellpadding="0">
          <tr><? echo tep_draw_form('search', FILENAME_CUSTOMERS, '', 'get'); ?>
            <td class="pageHeading"><? echo HEADING_TITLE; ?></td>
            <td class="pageHeading" align="right"><? echo tep_draw_separator('pixel_trans.gif', 1, HEADING_IMAGE_HEIGHT); ?></td>
            <td class="smallText" align="right"><? echo HEADING_TITLE_SEARCH . ' ' . tep_draw_input_field('search'); ?></td>
          </form></tr>
        </table></td>
      </tr>
      <tr>
          ...
          <tr>
            <td valign="top"><table border="0" width="100%" cellspacing="0" cellpadding="2">
              <tr class="dataTableHeadingRow">
                <td class="dataTableHeadingContent" align="center"><? echo TABLE_HEADING_FIRSTNAME; ?></td>
                <td class="dataTableHeadingContent" align="center"><a href="<?=tep_href_link(FILENAME_CUSTOMERS, tep_get_all_get_params(array('sort')));?>"><b><? echo "<big><b>".TABLE_HEADING_LASTNAME."</b></big> ".tep_image(DIR_WS_IMAGES . 'icon_arrow_up.gif', ''); ?></a></td>
                <td class="dataTableHeadingContent" align="center"><? echo "ID adhérent"; ?></td>
                <td class="dataTableHeadingContent" align="center">Email</td>
                <td class="dataTableHeadingContent" align="center"><a href="<?=tep_href_link(FILENAME_CUSTOMERS, tep_get_all_get_params(array('sort')).'sort=creation_date');?>"><? echo "<b><big>".TABLE_HEADING_ACCOUNT_CREATED."</big></b> ".tep_image(DIR_WS_IMAGES . 'icon_arrow_down.gif', ''); ?></a></td>
...
<?
    ...
    $customers_query_raw = "select c.customers_id, c.customers_lastname, c.customers_firstname, c.customers_email_address, a.entry_country_id from " . TABLE_CUSTOMERS . " c 
      ...
      WHERE (c.customers_id > 0 AND c.customers_firstname <> 'Password')";
    if (isset($HTTP_GET_VARS['sort']) && tep_not_null($HTTP_GET_VARS['sort'])) {
      $customers_query_raw .= " order by ci.customers_info_date_account_created DESC";
    } else {
      $customers_query_raw .= " order by c.customers_lastname, c.customers_firstname";
    }
    ...
    while ($customers = tep_db_fetch_array($customers_query)) {
      ...
      if ((!isset($HTTP_GET_VARS['cID']) || (isset($HTTP_GET_VARS['cID']) && ($HTTP_GET_VARS['cID'] == $customers['customers_id']))) && !isset($cInfo)) {
        ...
        $orders_query = tep_db_query("select count(*) as number_of_orders from " . TABLE_ORDERS . " where customers_id = '" . (int)$customers['customers_id'] . "'");
        $orders = tep_db_fetch_array($orders_query);

        if (($country)&&($reviews)&($info)) {
      ...
      $nb_adh += 1;
?>
                <td class="dataTableContent"><? echo $customers['customers_firstname']; ?></td>
                <td class="dataTableContent"><b><? echo $customers['customers_lastname']; ?></b></td>
                <td class="dataTableContent" align="center" nowrap>ID : <? echo $customers['customers_id']; ?></td>
                <td class="dataTableContent">&nbsp;<a href="mailto:<? echo $customers['customers_email_address']; ?>"><? echo $customers['customers_email_address']; ?></a>&nbsp;</td>
                <td class="dataTableContent" align="center"><? echo tep_date_short($info['date_account_created']); ?></td>
                <td class="dataTableContent" align="right"><? 
                  echo '<a href="' . tep_href_link(FILENAME_CUSTOMERS, tep_get_all_get_params(array('cID')) . 'cID=' . $customers['customers_id']) . '&action=edit' . '">' . tep_image(DIR_WS_ICONS . 'b_edit.png', ICON_PREVIEW) . '</a>&nbsp;';
                  if (isset($cInfo) && is_object($cInfo) && ($customers['customers_id'] == $cInfo->customers_id)) { echo tep_image(DIR_WS_IMAGES . 'icon_arrow_right.gif', ''); } else { echo '<a href="' . tep_href_link(FILENAME_CUSTOMERS, tep_get_all_get_params(array('cID')) . 'cID=' . $customers['customers_id']) . '">' . tep_image(DIR_WS_IMAGES . 'icon_info.gif', IMAGE_ICON_INFO) . '</a>'; } ?>&nbsp;</td>
              </tr>
<?
    }
?>
...
<?
if ($customers_query_numrows > MAX_DISPLAY_SEARCH_RESULTS) {
?>                
                  <tr>
                    ...
                    <td class="smallText" align="right"><? echo $customers_split->display_links($customers_query_numrows, MAX_DISPLAY_SEARCH_RESULTS, MAX_DISPLAY_PAGE_LINKS, $HTTP_GET_VARS['page'], tep_get_all_get_params(array('page', 'info', 'x', 'y', 'cID'))); ?></td>
                  </tr>
<?
} else {
?>                
                  <tr>
                    <td class="dataTableTotalRow" valign="top">Nombre d'adhérents</td>
                    <td class="dataTableTotalRow" align="right"><? echo $nb_adh;?></td>
                  </tr>
<?
...
?>
                  <tr>
                    <td align="right" colspan="2"><? echo '<a href="' . tep_href_link(FILENAME_CUSTOMERS) . '">' . tep_image_button('button_reset.gif', IMAGE_RESET) . '</a>'; ?></td>
                  </tr>
<?
    }
?>
...
<?
  $nom = $cInfo->customers_firstname . ' ' . $cInfo->customers_lastname;
  ...
  switch ($action) {
    case 'confirm':
      ...
      $contents[] = array('text' => TEXT_DELETE_INTRO . '<br><br><b>' . $nom . '</b>');
    ...
    default:
      if (isset($cInfo) && is_object($cInfo)) {
        ...
        $heading[] = array('text' => '<b>' . $cInfo->customers_firstname . ' ' . $cInfo->customers_lastname . '</b> (n° '.$cInfo->customers_id.')');

        $contents[] = array('align' => 'center', 'text' => '
          <a href="' . tep_href_link(FILENAME_CUSTOMERS, tep_get_all_get_params(array('cID', 'action')) . 'cID=' . $cInfo->customers_id . '&action=edit') . '">' . tep_image_button('button_edit.gif', 'Editer le compte de '.$nom) . '</a> 
          <a href="' . tep_href_link(FILENAME_CUSTOMERS, tep_get_all_get_params(array('cID', 'action')) . 'cID=' . $cInfo->customers_id . '&action=confirm') . '">' . tep_image_button('button_delete.gif', 'Effacer le compte de '.$nom) . '</a> 
          <br><a href="' . tep_href_link(FILENAME_ORDERS, 'cID=' . $cInfo->customers_id) . '">' . tep_image_button('button_orders.gif', 'Liste des commandes de '.$nom) . '</a> 
          <a href="javascript:showhide(\'new_order\');">' . tep_image_button('button_new_order.gif', 'Créer une nouvelle commande à '.$nom) . '</a> 
          <br><a href="' . tep_href_link(FILENAME_MAIL, 'selected_box=tools&customer=' . $cInfo->customers_email_address) . '">' . tep_image_button('button_email.gif', 'Envoyer un email à '.$nom) . '</a>
          ');
        $contents[] = array('text' => '<br><b>' . TEXT_DATE_ACCOUNT_CREATED . '</b> ' . getFormattedLongDate($cInfo->date_account_created));
        if (tep_date_short($cInfo->date_account_last_modified)) 
          $contents[] = array('text' => '<b>' . TEXT_DATE_ACCOUNT_LAST_MODIFIED . '</b> ' . getFormattedLongDate($cInfo->date_account_last_modified));

        if (tep_date_short($cInfo->date_last_logon)) 
          $contents[] = array('text' => '<b>' . TEXT_INFO_DATE_LAST_LOGON . '</b> '  . getFormattedLongDate($cInfo->date_last_logon));

        $contents[] = array('text' => '<br><b>' . TEXT_INFO_NUMBER_OF_LOGONS . '</b> ' . $cInfo->number_of_logons);
        $contents[] = array('text' => '<b>Nombre de commandes: </b> ' . $cInfo->number_of_orders);
        if ($cInfo->number_of_reviews>0) 
          $contents[] = array('text' => '<b>' . TEXT_INFO_NUMBER_OF_REVIEWS . '</b> ' . $cInfo->number_of_reviews);

        if (trim($cInfo->countries_name)) 
          $contents[] = array('text' => '<br><b>' . TEXT_INFO_COUNTRY . '</b> ' . trim($cInfo->countries_name));
        ...
        $contents[] = array('text' => '
          <div id="new_order" style="display:none;"><br><hr>'.
              tep_draw_form('new_order_form', FILENAME_ORDERS, tep_get_all_get_params(array('cID', 'action')) . 'cID=' . $cInfo->customers_id . '&action=new_order', 'post').
              '<big><center><b>Nouvelle commande</b></center></big><br>'.
              '<br><b>Type :</b> '.tep_draw_pull_down_menu('group_id', array(array('id' => '0', 'text' => 'Vente directe'), array('id' => '1', 'text' => 'Groupement d\'achat')), $gaID, ' onchange="showhide(\'new_order_vd\');showhide(\'new_order_ga\');"').
              '<div id="new_order_vd" style="display:none;"><br><b>Date de livraison (RDV) :</b> '.tep_draw_pull_down_menu('order_date_to_vd', $shipping_dates_array_vd, $last_order_date_vd).'</div>'.
              '<div id="new_order_ga" style="display:block;"><br><b>Date de livraison (GA) :</b> '.tep_draw_pull_down_menu('order_date_to_ga', $shipping_dates_array, $last_order_date).'</div><br><center>'.
              tep_image_submit('button_create.gif', 'Créer la commande') . '</center></form>
          </div>');
