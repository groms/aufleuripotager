...
*/
  ...
  $sql = "select p.products_id, pd.products_name, products_date_available as date_expected from " . TABLE_PRODUCTS . " p, " . TABLE_PRODUCTS_DESCRIPTION . " pd where to_days(products_date_available) >= to_days(now()) and p.products_id = pd.products_id and pd.language_id = '" . (int)$languages_id . "'";
  if (!clientCanBuyGA()) {
    // si l'adhérent n'est pas autorisé au grpt d'achat => et ben, pas de possibilité de le trouver !
    $sql .= " and p.group_id = 0 ";  
  }

  $sql .= " order by " . EXPECTED_PRODUCTS_FIELD . " " . EXPECTED_PRODUCTS_SORT . " limit " . MAX_DISPLAY_UPCOMING_PRODUCTS;

  $expected_query = tep_db_query($sql);
