...
*/
  if (!clientCanBuyGA()) {
    $vente_directe_only = " where group_id = 0 ";
  }

  $manufacturers_query = tep_db_query("select manufacturers_id, manufacturers_name, group_id from " . TABLE_MANUFACTURERS . $vente_directe_only . " order by group_id, manufacturers_name");
...
// Display a drop-down
      ...
      while ($manufacturers = tep_db_fetch_array($manufacturers_query)) {
        ...
        $manufacturers_name = ((strlen($m_name) > MAX_DISPLAY_MANUFACTURER_NAME_LEN) ? substr($m_name, 0, MAX_DISPLAY_MANUFACTURER_NAME_LEN) . '..' : $m_name);
