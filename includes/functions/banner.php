  ...
  function tep_activate_banners() {
    ...
    if (tep_db_num_rows($banners_query)) {
      while ($banners = tep_db_fetch_array($banners_query)) {
        if ((date('Y-m-d H:i:s') >= $banners['date_scheduled']) /*|| ($banners['date_scheduled'] == '') CJ*/) {
          exit;
...
// Display a banner from the specified group or banner id ($identifier)
  function tep_display_banner($action, $identifier = '') {
		// CJ 20090629 : ajout des ==========   if ($identifier != "") {   ============ 
    ...
    if ($action == 'dynamic') {
      ...
      if (is_array($identifier)) {
        $banner = $identifier;
		      ...
		      $banner_query = tep_db_query("select banners_id, banners_title, banners_image, banners_html_text from " . TABLE_BANNERS . " where status = '1'");
					while ($banner = tep_db_fetch_array($banner_query)) {
  					$info_box_contents[$row][0] = array(
							'text' => "<big><u><b>" . $banner['banners_title'] . "</b></u></big><br>" . $banner['banners_html_text']);

						if ($banner['banners_image'] != "") { 
							$info_box_contents[$row][1] = array(
								'text' => tep_image(DIR_WS_CATALOG_IMAGES . $banner['banners_image'], $banner['banners_title'], '32', '32'));
						}
      ...
      return '<b>TEP ERROR! (tep_display_banner(' . $action . ', ' . $identifier . ') -> Unknown $action parameter value - it must be either \'dynamic\' or \'static\'</b>';
		...
		if ($identifier != "") {
		  if (tep_not_null($banner['banners_html_text'])) {
		    ...
		    $banner_string = '<a href="' . tep_href_link(FILENAME_REDIRECT, 'action=banner&goto=' . $banner['banners_id']) . '" target="_blank">' . tep_image(DIR_WS_CATALOG_IMAGES . $banner['banners_image'], $banner['banners_title']) . '</a>';
		  }
