...
// redirect the customer to a friendly cookie-must-be-enabled page if cookies are disabled
    ...
    switch ($HTTP_GET_VARS['action']) {
      ...
      case 'add_product' :    if (isset($HTTP_POST_VARS['products_id']) && is_numeric($HTTP_POST_VARS['products_id'])) {
                                $qty = $cart->get_quantity(tep_get_uprid($HTTP_POST_VARS['products_id'], $HTTP_POST_VARS['id']));
                                if (isset($HTTP_POST_VARS['product_qty']) && $HTTP_POST_VARS['product_qty']>0) {
                                  $qty += $HTTP_POST_VARS['product_qty'];
                                } else {
                                  $qty += 1;
                                } 
                                $cart->add_cart($HTTP_POST_VARS['products_id'], $qty, $HTTP_POST_VARS['id']);
                              }
                              tep_redirect(tep_href_link($goto, tep_get_all_get_params($parameters)));
                              break;
...
// set which precautions should be checked
  ...
  define('WARN_CONFIG_WRITEABLE', 'false'); //CJ
